...
// REMOVED: fileToBase64 import

export function MessageInput({ onSend, onTyping }) {
  const [text, setText]               = useState('');
  const [pendingFile, setPendingFile] = useState(null); // stores raw File
  ...
  const handleFileSelect = (e) => {
    ...
    setFileError('');
    // Store raw File directly — NO base64 conversion
    setPendingFile(file);
    e.target.value = '';
  };
  ...
  const handleSend = () => {
    if (pendingFile) {
      onSend('', pendingFile); // pass raw File object
      setPendingFile(null);
      return;
    }
    if (!text.trim()) return;
